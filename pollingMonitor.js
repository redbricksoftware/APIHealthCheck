...
function initTimers(apiConfigList) {
    for (var i = 0; i < apiConfigList.length; i++) {
        ...
        addDynamicFunction(apiConfigList[i].configID, apiConfigList[i].name, apiConfigList[i].pollFrequencyInSeconds, apiConfigList[i].uri);
    }
...
function addTimer(apiConfig) {
    if (dyn_functions[apiConfig.configID]) {
        ...
        addDynamicFunction(apiConfig.configID, apiConfig.name, apiConfig.pollFrequencyInSeconds, apiConfig.uri);
    }
}
//remove a timer
...
function respCallback(id, res, normalizeFunction, start, end) {
    ...
    var responseInMS = end.diff(start);
